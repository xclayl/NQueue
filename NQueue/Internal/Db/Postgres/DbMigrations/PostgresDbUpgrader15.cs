...
public class PostgresDbUpgrader15
{
	...
	public async ValueTask Upgrade(DbTransaction tran, bool isCitus)
	{
		...
		var sql = @"
			ALTER TABLE NQueue.BlockingMessage 		ADD COLUMN IF NOT EXISTS ExternalLockId text NULL;

			ALTER TABLE NQueue.BlockingMessage 		ALTER COLUMN BlockingWorkItemId DROP NOT NULL;
			ALTER TABLE NQueue.BlockingMessage 		ALTER COLUMN BlockingShard DROP NOT NULL;
        ";
...
begin
	...
	INSERT INTO NQueue.BlockingMessage (QueueShard, QueueMaxShards, QueueName, IsCreatingBlock, ExternalLockId)
	VALUES (pShard, pMaxShards, pQueueName, TRUE, pExternalLockId);
