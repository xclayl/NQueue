...
public class PostgresDbUpgrader18
{
...
begin
    ...
    IF vQueueName IS NULL THEN
		RAISE EXCEPTION 'Work Item % could not be found', pWorkItemID;
	END IF;
	...
	IF vWorkItemExternalLockIdWhenComplete IS NOT NULL THEN 
		-- the next work item must be noop: to block the parent (vWorkItemBlockingQueueName) or next work-item in the queue
		...
		IF vWorkItemBlockingQueueName IS NOT NULL THEN
			INSERT INTO NQueue.BlockingMessage (QueueShard, QueueMaxShards, QueueName, IsCreatingBlock, BlockingWorkItemId, BlockingShard)
				VALUES (vWorkItemBlockingQueueShard, pMaxShards, vWorkItemBlockingQueueName, TRUE, vNextWorkItemID, pShard);
		END IF;

		UPDATE
			NQueue.Queue q
		SET ExternalLockId = vWorkItemExternalLockIdWhenComplete,
			LockedUntil = vNextCreatedAt,
